...
parser.add_argument('--train_vars_limit', action='store_true',help='limit training vars')
...
def main(tpu_cluster=None):
    ...
    with tf.Session(tpu_cluster, config=config) as sess:
        ...
        train_vars = [v for v in all_vars if 'n_1' in v.name] if args.train_lur else all_vars
        print(train_vars)
